...
6. **TAT is opt-in per matter** — there is no global default to seed. Set it per step type (`STEP1`/`STEP2`/`CLIENT_APPROVAL`/`FILING`) on the "New Matter" form when creating each matter; a step left unchecked there has no due date.
