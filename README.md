...
- `frontend/` - Next.js app (TypeScript, App Router), deployed to Vercel, talking directly to Supabase (`@supabase/supabase-js`).
...
2. `cd frontend && npm install`, copy `.env.local.example` to `.env.local`, set `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_ANON_KEY`, then `npm run dev`.
